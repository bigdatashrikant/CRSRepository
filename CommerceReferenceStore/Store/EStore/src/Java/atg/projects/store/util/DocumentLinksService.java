 ...
 *  
 * @author ATG
 * @version $Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/util/DocumentLinksService.java#2 $$Change: 877954 $
 * @updated $DateTime: 2014/03/25 06:51:13 $$Author: releng $
 */ 
public class DocumentLinksService extends GenericService {  
  
  /** Class version string */
  public static final String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/util/DocumentLinksService.java#2 $$Change: 877954 $";
  ...
  /**
   * @return Returns true if the International module is running
   */
  ...
  /**
   * @param pInternational Set the value for international. This should be true
   * in the International module layer to indicate its running.
   */
  public void setInternational(boolean pInternational) {
    mInternational = pInternational;
  }
  ...
  /**
   * <p>
   *   Gets an available <tt>Locale</tt> that isn't the same as the 
   *   current <tt>Locale</tt> and associates it with the 'alternate' 
   *   canonical link reference.
   * </p>
   * <p>
   *   For example, if the current locale is 'en_US' but the current site
   *   also defines the Spanish language; Spanish will be used as the
   *   alternate canonical link and the following will be returned in a Map: 
   *   <ul>
   *     <li>key: "alternate", value: "es_US"</li>
   *   </ul>  
   * </p>
   *
   * @param pRequest
   ...
   * @return 
   *   An available 'alternate' canonical link, otherwise null.
   */
  public Map<String,Locale> getAlternateLink(DynamoHttpServletRequest pRequest, 
    RequestLocale pRequestLocale) {
    ...
    List<String> languages = (List<String>) 
      currentSite.getPropertyValue(LANGUAGES);      
    
    Locale currentLocale =
      requestLocale.discernRequestLocale(pRequest, requestLocale);
    
    if (languages != null && currentLocale != null) {
      Map<String,Locale> result = new HashMap<String,Locale>();
      
      String countryCode = 
        (String) currentSite.getPropertyValue(DEFAULT_COUNTRY);

      String currentLanguage = currentLocale.getLanguage();
      
      if (currentLanguage != null) {
        // Iterating through available locales. If a locale isn't equal to
        // the current locale, put alternate link into the map.
        for (String language : languages) {
          if (!currentLanguage.equalsIgnoreCase(language)) {
            Locale locale = new Locale(language, countryCode);
            result.put(ALTERNATE, locale);
          }
        }
        return result;
      }
  ...
  public Map getOtherLinks(RepositoryItem pCurrentCategory,
    ...
    // Check if International module is being used. If not, there will be no 
    // other languages and no alternate links.
    if (isInternational()) {
      if (pCurrentCategory != null || pCurrentProduct != null) {
        
        Map alternateLink = getAlternateLink(pRequest, pRequestLocale);
        
        if (alternateLink != null && alternateLink.size() > 0) {
          links.put(ALTERNATE, alternateLink.get(ALTERNATE));
        }
      }
