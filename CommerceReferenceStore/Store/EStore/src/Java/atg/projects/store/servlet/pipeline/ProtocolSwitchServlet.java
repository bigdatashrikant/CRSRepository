...
public class ProtocolSwitchServlet extends InsertableServletImpl {
  ...
  protected String getNonSecureUrl(String pUrl) {
    ...
    buffer.append("://");
    buffer.append(getAddressForUrl(getHostName()));
  ...
  protected String getSecureUrl(String pUrl) {
    ...
    // XXX - make property
    buffer.append(getAddressForUrl(getSecureHostName()));
    if (httpsPort != 0) {
   ...
   * 
   * @param pHostname The ip address to be wrapped
   * 
   * @return The ip address string for the given ip address
   */
  public String getAddressForUrl(String pHostname) {
    if (pHostname != null

    && (!(pHostname.contains("[") && pHostname.contains("]")))
        && IPAddressUtil.isIPv6LiteralAddress(pHostname)) {
      pHostname = "[" + pHostname + "]";
    }
    return pHostname;
  }
