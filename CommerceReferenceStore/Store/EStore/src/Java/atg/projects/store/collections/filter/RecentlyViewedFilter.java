 ...
 * @version $Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/collections/filter/RecentlyViewedFilter.java#2 $$Change: 877954 $
...
public class RecentlyViewedFilter extends CachedCollectionFilter
{
  ...
  public static String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/collections/filter/RecentlyViewedFilter.java#2 $$Change: 877954 $";
                                                  ...
                                                  Map pExtraParameters) throws FilterException 
  {
    ...
    // Make sure the passed collection is a populated List<RepositoryItem>
    if(!(pUnfilteredCollection instanceof List) 
    || pUnfilteredCollection.isEmpty() 
    || !(((List)pUnfilteredCollection).get(0) instanceof RepositoryItem))
    {
      if (isLoggingDebug()) {
        logDebug("The unfiltered collection should be of type List<RepositoryItem>. Returning empty list.");
      }
      return resultCollection;
    }
    
    // If we get here we know pUnfilteredCollection contains items
    ...
    for (int i = 0; i < recentlyViewed.size(); i++) {
      ...
      // If the siteId variable is null, an empty string must be used in order for it's
      ...
      try {
        ...
        if (getCatalogTools().findProduct(product.getRepositoryId()) == null){
          if (isLoggingDebug()) {
