 ...
 *
 * @author ATG
 * @version $Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/profile/StoreProfileFormHandler.java#2 $$Change: 877954 $
...
public class StoreProfileFormHandler extends CommerceProfileFormHandler {
  ...
  public static final String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/profile/StoreProfileFormHandler.java#2 $$Change: 877954 $";
  ...
  //Invalid date message key.
  protected static final String MSG_INVALID_DATE_ENTERED = "invalidDateEntered";
...
/**
    ...
    int pReturnCode)
  {
    ...
    switch (pReturnCode) {
      case ExtendableCreditCardTools.CARD_TYPE_NOT_VALID :
        path.append(getStorePropertyManager().getCreditCardTypePropertyName());
        break;
      ...
      case ExtendableCreditCardTools.CARD_NUMBER_NOT_VALID :
        path.append(getStorePropertyManager().getCreditCardNumberPropertyName());
        break;

      case ExtendableCreditCardTools.CARD_EXPIRED :
      case ExtendableCreditCardTools.CARD_EXP_DATE_NOT_VALID :
        path.append(getStorePropertyManager().getCreditCardExpirationMonthPropertyName());
        break;
   ...
   */
  protected void validateBirthDate(DynamoHttpServletRequest pRequest) {
    Integer day = null;
    Integer month = null;
    Integer year = null;
    
    byte notSetPropertiesCount = 0;
    try {
      day = Integer.parseInt(mDate);
    }
    catch (NumberFormatException nfe){
      notSetPropertiesCount++;
    }
    ...
    if (notSetPropertiesCount == 3) {
      /*
       *  All birth day properies are not set. As they are
       *  optional no error will be displayed
       */
      return;        
    }
    /*
     *  Some (not all) date properties are set, however it won't be possible
     *  to form valid date from them
     */
    else if (notSetPropertiesCount > 0 ) {
      addFormException(MSG_INVALID_DATE_ENTERED, getAbsoluteName(), pRequest);  
      return;
    }
