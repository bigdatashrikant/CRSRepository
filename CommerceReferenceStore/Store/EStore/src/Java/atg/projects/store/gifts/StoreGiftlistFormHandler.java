 ...
 *
 * @author ATG
 * @version $Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/gifts/StoreGiftlistFormHandler.java#2 $
...
public class StoreGiftlistFormHandler extends GiftlistFormHandler { 
  ...
  public static final String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/gifts/StoreGiftlistFormHandler.java#2 $$Change: 877954 $";
   ...
   */
  @Override
  public void postMoveItemsFromCart(DynamoHttpServletRequest pRequest, DynamoHttpServletResponse pResponse) 
    throws ServletException, IOException {

    StoreOrderImpl order = (StoreOrderImpl) getOrder();
    
    try {
      // Remove gift services when the only commerce item left in the order is a GiftWrapCommerceItem.
      ((StoreGiftlistManager)getGiftlistManager()).removeInvalidGiftServices(order);
    }
    catch (CommerceException ce) {
      vlogError(ce, "There was a problem updating order: {0}", order.getId());
    }
  }
