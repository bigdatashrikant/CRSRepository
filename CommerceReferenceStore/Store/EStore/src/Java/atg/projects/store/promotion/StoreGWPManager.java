 ...
 * 
 * @author ATG
 * @version $Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/promotion/StoreGWPManager.java#2 $$Change: 877954 $
...
public class StoreGWPManager extends GWPManager {
  ...
  public static final String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/promotion/StoreGWPManager.java#2 $$Change: 877954 $";
    ...
    try {
      ...
      if (selectionChoice != null) {
        ...
        if(isValid && (availabilityStatus == getInventoryManager().getAvailabilityStatusInStockValue() ||
          ...
          if(isLoggingDebug()) {
            logDebug("Auto add item is out of stock or has invalid dates, set quantity " + pQuantityToAdd + " as failed");
          }
          markGiftAddAsFailed(pOrderMarker, 
            manager.getFailedQuantity(pOrderMarker) + pQuantityToAdd);
        }
      }
    }
    catch(CommerceException e){
   ...
   */
  protected void markGiftAddAsFailed(RepositoryItem pOrderMarker,
    long pQuantity) throws PricingException
  { 
    try {
      getGwpMarkerManager().setFailedQuantity(pOrderMarker, pQuantity);
    } 
      ...
      if (pOrderMarker != null) { 
        vlogError(pOrderMarker.toString());
      }
