...
public class PriceSorter extends GenericService implements PropertySorter{
  ...
  public static String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/sort/PriceSorter.java#3 $$Change: 889327 $";
  ...
  /**
   * This method is provided so that subclasses can specify the price
   * list to be used in the sort operation. The implementation of this
   * method always returns null, which causes the sort to use the
   * price list on the current user profile.
   *
   * @return the price list or null
   */
  public RepositoryItem determinePriceList() { return null; }
  
  /**
   * This method is provided so that subclasses can specify the sale
   * price list to be used in the sort operation. The implementation
   * of this method always returns null, which causes causes the sort
   * to use the sale price list on the current user profile.
   *
   * @return the sale price list or null 
   */
  public RepositoryItem determineSalePriceList() { return null; }
    ...
    for(Object product : pItems){
      if(product instanceof RepositoryItem){
        ...
        if(getPricingTools().isUsingPriceLists()){
          try {
            highestPrice = getPricingTools().retrieveHighestPriceListPrice(
              (RepositoryItem)product, determinePriceList(), determineSalePriceList());
            lowestPrice = getPricingTools().retrieveLowestPriceListPrice(
              (RepositoryItem)product, determinePriceList(), determineSalePriceList());
          }
