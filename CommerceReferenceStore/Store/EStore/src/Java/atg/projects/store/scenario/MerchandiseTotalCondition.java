...
public class MerchandiseTotalCondition extends ExpressionFilter {
  ...
  public static final String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/11.1/EStore/src/atg/projects/store/scenario/MerchandiseTotalCondition.java#2 $$Change: 877954 $";
  ...
  /** 
   * The comparison type being performed on the item totals as per above constants.
   */
  int mComparisonType;
  ...
  /** Scenario PMDL cache. **/
  AbstractCache mScenarioPMDLCache;
  ...
  public void initialize(String pOperator, Expression[] pOperands)
    ...
    Object typeVal = exComparisonType.getValue(null);
    if( !(typeVal instanceof Integer) ){
      String type = typeVal == null ? "null" : typeVal.getClass().getName();
      throw new ProcessException("Test type had unexpected type: " + type);
    }
    else{
      mComparisonType = ((Integer)typeVal).intValue();
    }
    ...
    Object amountVal = exCompareAmount.getValue(null);
    if( !(amountVal instanceof Double) ){
      String type = amountVal == null ? "null" : amountVal.getClass().getName();
      throw new ProcessException("Amount had unexpected type: " + type);
    }
    else{
      mCompareAmount = ((Double)amountVal).doubleValue();
    }
  ...
  protected boolean evaluatePMDLRule(ProcessExecutionContext pContext, Order pOrder, CommerceItem pItem)
    ...
    if (!(ret instanceof Boolean)) {
      throw new ProcessException(MessageFormat.format(Constants.FILTER_BAD_EVAL,
          new Object[] { mPMDLString, ret.getClass().getName() }));
    }
