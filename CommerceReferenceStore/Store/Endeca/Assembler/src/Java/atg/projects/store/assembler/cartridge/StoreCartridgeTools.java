 ...
 * @version $Id: //hosting-blueprint/B2CBlueprint/version/11.1/Endeca/Assembler/src/atg/projects/store/assembler/cartridge/StoreCartridgeTools.java#3 $$Change: 885024 $
 * @updated $DateTime: 2014/04/23 22:55:51 $$Author: jsiddaga $
 */
public class StoreCartridgeTools {
  ...
  protected static final String CLASS_VERSION = 
    "$Id: //hosting-blueprint/B2CBlueprint/version/11.1/Endeca/Assembler/src/atg/projects/store/assembler/cartridge/StoreCartridgeTools.java#3 $$Change: 885024 $";
   ...
   * 
   * @param pRecordFilters - The List of RecordFilters to be updated.
   * @param pRecordFilterBuilders - The RecordFilterBuilders that will generate RecordFilters to be added to pRecordFilters.
   * 
   * @return a new RecordFilter list consisting of pRecordFilters and the RecordFilters generated by pRecordFilterBuilders. 
   */
  public List<String> updateRecordFilters(List<String> pRecordFilters, 
                                          RecordFilterBuilder[] pRecordFilterBuilders) {
    
    List<String> allRecordFilters = pRecordFilters;
    
    if(pRecordFilters == null) {
      allRecordFilters = new ArrayList<String>();
    }
    
    if(pRecordFilterBuilders == null) {
      return pRecordFilters;
    }
    
    // Generate the RecordFilters.
    List<String> generatedRecordFilters = new ArrayList<String>();
    
    for (RecordFilterBuilder builder : pRecordFilterBuilders) {
      generatedRecordFilters.add(builder.buildRecordFilter());
    }
      
    // Ensure that no generated RecordFilters are added as duplicates.
    for (String generatedRecordFilter : generatedRecordFilters) {
      boolean addToList = true;

      for (String recordFilter : allRecordFilters) {
        if (recordFilter.equals(generatedRecordFilter)) {
          addToList = false;
          break;
        }
      }
      ...
      if (addToList) {
        allRecordFilters.add(generatedRecordFilter);
      }
    }
    
    return allRecordFilters;
  }
