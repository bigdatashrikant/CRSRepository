...
public class SGCIRelationshipPropertyCustomizer implements PropertyCustomizer {
  ...
  public static String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/11.1/Mobile/src/atg/service/filter/bean/SGCIRelationshipPropertyCustomizer.java#3 $$Change: 889821 $";
  ...
  public Object getPropertyValue(Object pTargetObject, String pPropertyName,
    ...
    for (Object element : sgCiRels) {
      ...
      if (ci.getPriceInfo() != null) {
        ...
        // Overwrite the priceInfo.amount value with the amount for the current
        // shipping group:
        wrappedSgCiRel.putSubValue("commerceItem.priceInfo.amount", getTotalAmount(currentPriceDetails));
  ...
  /**
   * Get the total of all amount values for each DetailedItemPriceInfo instance.
   *
   * @param pPriceDetails A possibly empty list of DetailedItemPriceInfo objects.
   * @return the total of all amount properties of each DetailedItemPriceInfo instance
   */
  double getTotalAmount(List<?> pPriceDetails)
  {
    double result = 0.0;

    for (Object obj : pPriceDetails) {
      if (obj != null) {
        DetailedItemPriceInfo dipi = (DetailedItemPriceInfo) obj;
        result += dipi.getAmount();
      }
    }
